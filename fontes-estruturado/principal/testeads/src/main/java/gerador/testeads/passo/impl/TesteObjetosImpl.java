	...
	protected boolean executaCustom() {
		...
		try {
			...
			for (TesteAds item : lista) {
				...
				this.consultaSql(googleAdsClient, item.getSql(), item.getTabela(), 3);
			}
			...
			*/
			...
			String query = "SELECT keyword_text, keyword_match_type FROM keyword_view WHERE search_query LIKE 'como fazer%' AND search_query >= 200000 ORDER BY search_query DESC limit 50";
			this.consultaSql(googleAdsClient, query);
			...
			//customer_conversion_goal - 2 linha
