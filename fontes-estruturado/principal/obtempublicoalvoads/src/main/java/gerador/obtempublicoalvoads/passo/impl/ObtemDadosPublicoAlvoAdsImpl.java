...
public class ObtemDadosPublicoAlvoAdsImpl extends ObtemDadosPublicoAlvoAds {
	...
	protected boolean executaCustom(ContaGoogle contaCorrente) {
		...
		List<PublicoAlvoAdsDiario> lista = new ArrayList<PublicoAlvoAdsDiario>();
		...
		for (GoogleAdsRow linha : aux.getLista()) {
			PublicoAlvoAdsDiario item = new PublicoAlvoAdsDiario();
			processaLinha(linha,item);
			lista.add(item);
		}
		this.saidaListaPublico = lista;
		return true;
	}
