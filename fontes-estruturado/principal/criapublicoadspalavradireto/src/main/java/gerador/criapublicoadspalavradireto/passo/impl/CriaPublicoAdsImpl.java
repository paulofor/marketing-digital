...
public class CriaPublicoAdsImpl extends CriaPublicoAds {

	@Override
	protected boolean executaCustom(PublicoAlvoAdsPalavra publicoCorrente) {
		GoogleAdsClient googleAdsClient = null;
		try {
			long codigoUsuario = new Long(publicoCorrente.getContaGoogle().getIdAds().replace("-", ""));
			googleAdsClient = GoogleAdsClient.newBuilder().fromPropertiesFile().build();
			String resourceName = criaSegmento(googleAdsClient, codigoUsuario, publicoCorrente);
			this.saidaPublicoCorrente = publicoCorrente;
			this.saidaPublicoCorrente.setResourceName(resourceName);
